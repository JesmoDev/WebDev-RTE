...
/**
 * @element uui-rich-text-editor
 */
export class UUIRichTextEditorElement extends LitElement {
  static styles = [
    css`
      :host {
        width: 100%;
  ...
  @state()
  private _currentMenu: MenuBase | null = null;

  firstUpdated() {
    const mountElement = this.shadowRoot!.getElementById('editor') as Element;
    const hoverMenu = this.shadowRoot!.getElementById(
      'hover-menu'
    ) as HTMLElement;
  ...
  public openMenu(tag: string): void {
    ...
    const mountElement = this.shadowRoot!.getElementById(
      'editor'
    ) as HTMLElement;
